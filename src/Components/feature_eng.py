...
class FeatureEnginerring():
    ...
    def Build_customer_dataset(self):
        try:
            ...
            customer = self.data.groupby('Customer_ID', group_keys=False)
    ...
    def process_baskets(self):
        ...
        transactions = basket['StockCode'].apply(lambda x: list(set(x))).tolist()
        
        te = TransactionEncoder()
        te_ary = te.fit_transform(transactions, sparse=True)
    ...
    def association_rules(self):
        ...
        basket_bool = self.process_baskets()
        itemsets, rules = apriori(basket_bool, min_support=0.01, min_confidence=0.2, max_length=3)
    ...
    def Run(self):
        try:
            ...
            basket = self.Build_baskets_dataset()
            ruleset = self.association_rules()
